...
private: 
	...
	bool checkValidationLayerSupport() {
		...
		// 检查是否所有validationLayers列表中的校验层都能在availableLayers列表中找到
	...
	bool isDeviceSuitable(VkPhysicalDevice device) { 
		...
		bool extensionsSupproted = checkDeviceExtensionSupport(device); // 检查是否有交换链的扩展

		bool swapChainAdequate = false; // 检查交换链是否支持至少一种图像格式以及支持窗口表面的呈现模式
		if (extensionsSupproted) {
			SwapChainSupportDetails swapChainSupport = querySwapChainSupport(device);
			swapChainAdequate = !swapChainSupport.formats.empty() && !swapChainSupport.presentModes.empty();
		}

		return indices.isComplete() && extensionsSupproted && swapChainAdequate;
	}
	...
	void createLogicalDevice() {
		...
		float queuePriority = 1.0f; // Vulkan需要我们显式地给队列赋予优先级
	...
	void createSwapChain() {
		...
		uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1; // 最小图像数+1实现三倍缓冲
	...
	void createCommandPool() {
		...
		poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamily; // 每个指令池对象分配的指令缓存只能提交给一个特定队列
	...
	void createCommandBuffer() {
		...
		for (size_t imageIndex = 0; imageIndex < commandBuffers.size(); imageIndex++) {
